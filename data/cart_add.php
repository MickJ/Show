...
//SQL2：根据uname查询uid
$sql = "SELECT uid FROM oppo_user WHERE uname='$uname'";
...
$sql = "SELECT * FROM oppo_cart_detail WHERE cartId='$cid' AND productId='$pid'";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_assoc($result);
if($row){ //已经购买过该商品
	$count = $row['count']; 
	$count++;
	//SQL6：修改购买数量
	$sql = "UPDATE oppo_cart_detail SET count='$count' WHERE cartId='$cid' AND productId='$pid'";
	mysqli_query($conn,$sql);
	
}else {  //没有购买过该商品
	//SQL7: 插入一行购买记录
	$sql = "INSERT INTO oppo_cart_detail VALUES(NULL,'$cid','$pid',1)";
	mysqli_query($conn,$sql);
	$count = 1;
}
$output['count']=$count;
